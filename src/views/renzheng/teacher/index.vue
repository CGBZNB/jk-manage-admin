       ...
       <el-table-column label="登录账号" min-width="100px">
        <template slot-scope="{row}">
           <span>{{ row.userLogin.name==null?row.userLogin.account:row.userLogin.name}}</span>
             ...
             <el-table-column label="驾校" min-width="100px">
        <template slot-scope="{row}">
...
import {rzsteacher,delteacher}from '@/api/info'
   ...
   async getList() {
      ...
      try{
       let params={
         ...
         await this.$store.dispatch("getteachers",params)

         this.list=this.$store.state.teacher.teachers
    ...
    handleUpdate(row) {
     ...
     async updateData(temp) {
      try{
         let result= await rzsteacher({sid:temp.id})
          if(result.code==201){
           this.$notify({
              title: 'Success',
              message: '认证成功',
      ...
      try {
         ...
         let result=  await delteacher({tid:row.id})
         if(result.code==201){
           this.$notify({
        title: 'Success',
