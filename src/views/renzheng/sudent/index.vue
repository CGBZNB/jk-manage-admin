  ...
  <div class="app-container">
      ...
      <el-table-column label="电话" width="120px">
        <template slot-scope="{row}">
           <span>{{ row.phone }}</span>
            ...
            <el-table-column label="年龄" width="80px">
        <template slot-scope="{row}">
           <span>{{ row.age }}</span>
        </template>
      </el-table-column>
       <el-table-column label="身份证" width="180px">
        <template slot-scope="{row}">
           <span>{{row.idCard}}</span>
        </template>
      </el-table-column>
       <el-table-column label="登录账号" width="100px">
        <template slot-scope="{row}">
           <span>{{ row.userLogin.name==null?row.userLogin.account:row.userLogin.name}}</span>
        </template>
      </el-table-column>
                   <el-table-column label="驾校" width="100px">
        <template slot-scope="{row}">
           <span>{{ row.school.name}}</span>
        </template>
      </el-table-column>
            </el-table-column>
                   <el-table-column label="教练" width="100px">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
          ...
          <el-input :value="temp.sex=='Male'?'男':'女'" disabled/>
        </el-form-item>
      <el-form-item label="账号" prop="title">
          ...
          <el-input v-model="temp.school.name" disabled/>
        </el-form-item>
                        <el-form-item label="教练" prop="title">
          <el-input v-model="temp.teacher.name" disabled/>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogStatus==='create'?createData():updateData(temp)">
         提交
        </el-button>
      </div>
    </el-dialog>
...
import {rzstudent,deltudent}from '@/api/info'
  ...
  methods: {
   async getList() {
      ...
      try{
         ...
         await this.$store.dispatch("getstudents",params)
        
         this.list=this.$store.state.student.students
   ...
   async updateData(temp) {
      try{
         let result= await rzstudent({sid:temp.id})
          if(result.code==201){
           this.$notify({
              title: 'Success',
              message: '认证成功',
    ...
    async handleDelete(row, index) {
      try {
         ...
         let result=  await deltudent({sid:row.id})
         if(result.code==201){
           this.$notify({
        title: 'Success',
